...
export type SortType = 'newest' | 'oldest' | 'alphabetical' | 'priority';
        ...
        {isShared && (
          <div className="bg-gradient-to-r from-blue-100 to-purple-100 dark:from-blue-900/20 dark:to-purple-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-foreground">Remind Me Love</h3>
                <p className="text-sm text-muted-foreground">
                  Tasks and goals to accomplish together with my baby
                </p>
              </div>
            </div>
          </div>
        ...
        {sortedTodos.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground">
              {filter === 'all' 
                ? (isShared ? 'No family goals yet' : 'No tasks yet')
                : `No ${filter} ${isShared ? 'family goals' : 'tasks'}`
              }
            </p>
            {isShared && filter === 'all' && (
              <p className="text-sm text-muted-foreground mt-2">
                Create goals that Giscard and CeAnna can work on together!
              </p>
            )}
          </div>
        )}
